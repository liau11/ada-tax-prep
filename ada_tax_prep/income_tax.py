...
def calculate_deducted_income(income, deductions, standard_deduction):
    summed_deductions = sum(
        (deductions.get(category, 0) for category in DEDUCTION_CATEGORIES)
    )
...
def calculate_tax_liability_2020(income, deductions):
    return calculate_tax_liability(
        income, deductions, STANDARD_DEDUCTION_2020, TAX_BRACKETS_2020
    )
...
class TaxPayer:
    ...
    def calculate_return(self, standard_deduction, brackets):
        deducted_income = calculate_deducted_income(
            self.income, self.deductions, standard_deduction
        )
    ...
    def calculate_return_2020(self):
        deducted_income = calculate_deducted_income(
            self.income, self.deductions, STANDARD_DEDUCTION_2020
        )
